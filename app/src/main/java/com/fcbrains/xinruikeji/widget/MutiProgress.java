package com.fcbrains.xinruikeji.widget;
...
import com.fcbrains.xinruikeji.R;
...
public class MutiProgress extends View{
    ...
    private int currNodeNO=0;  //当前进行到的节点编号。从0开始计算
    private int currNodeState; //当前进行到的节点编号所对应的状态 0：失败  1：成功

    //    private int textSize;  //字体大小
    public int[] state = new int[nodesNum];//完成节点的状态
    Context mContext;
    ...
    public void setCurrNodeNO(int currNodeNO) {
        this.currNodeNO = currNodeNO;
    }
...
//        mCanvas.drawLine(nodeRadius, mHeight/2, mWidth-nodeRadius, mHeight/2, mPaint);  //线段2端去掉nodeRadius
        canvas.drawLine(nodeRadius, mHeight/2, nodes.get(currNodeNO).mPoint.x + nodeRadius, nodes.get(currNodeNO).mPoint.y + nodeRadius, mPaint);  //线段2端去掉nodeRadius
        //后半截线段
        mPaint.setColor(Color.parseColor("#dddddd"));
        canvas.drawLine(nodes.get(currNodeNO).mPoint.x +nodeRadius, nodes.get(currNodeNO).mPoint.y + nodeRadius, mWidth-nodeRadius, mHeight/2, mPaint);  //线段2端去掉nodeRadius
    }
    class Node
    {
        Point mPoint;
